...
for(i in 1:10) {
  ...
  conservation <- train(label ~ ., data = train_data, 
                        method = "nb", #随机森林："rf" #朴素贝叶斯："nb" #通用线性模型："glm"
...
FPR_nb <- 1 - specificities
FNR_nb <- 1 - sensitivities

write.csv(FPR_nb, "../datas/FPR_nb.csv")
write.csv(FNR_nb, "../datas/FNR_nb.csv")
write.csv(conservation$results[2,]$ROC, "../datas/auc_value_nb.csv")
