...
for(i in 1:10) {
  ...
  conservation <- train(label ~ ., data = train_data, 
                        method = "knn", #随机森林："rf" #朴素贝叶斯："nb" #通用线性模型："glm"
...
FPR_knn <- 1 - specificities
FNR_knn <- 1 - sensitivities

write.csv(FPR_knn, "../datas/FPR_knn.csv")
write.csv(FNR_knn, "../datas/FNR_knn.csv")
write.csv(conservation$results[2,]$ROC, "../datas/auc_value_knn.csv")
