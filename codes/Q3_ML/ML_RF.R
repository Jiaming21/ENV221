...
for(i in 1:10) {
  ...
  conservation <- train(label ~ ., data = train_data, 
                        method = "rf", #随机森林："rf" #朴素贝叶斯："nb" #通用线性模型："glm"
...
FPR_rf <- 1 - specificities
FNR_rf <- 1 - sensitivities

write.csv(FPR_rf, "../datas/FPR_rf.csv")
write.csv(FNR_rf, "../datas/FNR_rf.csv")
write.csv(conservation$results[2,]$ROC, "../datas/auc_value_rf.csv")
