...
for(i in 1:10) {
  ...
  conservation <- train(label ~ ., data = train_data, 
                        method = "pcaNNet", #随机森林："rf" #朴素贝叶斯："nb" #通用线性模型："glm"
...
FPR_pcaNNet <- 1 - specificities
FNR_pcaNNet <- 1 - sensitivities

write.csv(FPR_pcaNNet, "../datas/FPR_pcaNNet.csv")
write.csv(FNR_pcaNNet, "../datas/FNR_pcaNNet.csv")
write.csv(conservation$results[2,]$ROC, "../datas/auc_value_pcaNNet.csv")
