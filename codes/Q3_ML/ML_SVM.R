...
for(i in 1:10) {
  ...
  conservation <- train(label ~ ., data = train_data, 
                        method = "svmRadial", #随机森林："rf" #朴素贝叶斯："nb" #通用线性模型："glm"
...
auc_value_svm <- round(roc_data$auc, 4)

# plot(roc_data, main = paste("Round", i, "- ROC Curve (AUC =", auc_value, ")"))
# legend("bottomright", legend = paste("AUC =", auc_value), col = "black", lty = 1)

# Save ROC Curve to a file
# png(paste("ROC_Round_", i, ".png"), width = 600, height = 600)
# plot(roc_data, main = paste("Round", i, "- ROC Curve (AUC =", auc_value, ")"))
# dev.off()
...
FPR_svm <- 1 - specificities
FNR_svm <- 1 - sensitivities

write.csv(FPR_svm, "../datas/FPR_svm.csv")
write.csv(FNR_svm, "../datas/FNR_svm.csv")
write.csv(conservation$results[2,]$ROC, "../datas/auc_value_svm.csv")
