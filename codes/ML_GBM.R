...
for(i in 1:10) {
  ...
  conservation <- train(label ~ ., data = train_data, 
                        method = "gbm", #随机森林："rf" #朴素贝叶斯："nb" #通用线性模型："glm"
...
auc_value_GBM <- round(roc_data$auc, 4)

sensitivities <- roc_data$sensitivities
specificities <- 1 - roc_data$specificities

FPR_GBM <- 1 - specificities
FNR_GBM <- 1 - sensitivities

write.csv(FPR_GBM, "../datas/FPR_GBM.csv")
write.csv(FNR_GBM, "../datas/FNR_GBM.csv")
write.csv(conservation$results[2,]$ROC, "../datas/auc_value_gbm.csv")
