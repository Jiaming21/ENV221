...
for(i in 1:10) {
  ...
  conservation <- train(label ~ ., data = train_data, 
                        method = "nnet", #随机森林："rf" #朴素贝叶斯："nb" #通用线性模型："glm"
...
FPR_nnet <- 1 - specificities
FNR_nnet <- 1 - sensitivities

write.csv(FPR_nnet, "../datas/FPR_nnet.csv")
write.csv(FNR_nnet, "../datas/FNR_nnet.csv")
write.csv(conservation$results[2,]$ROC, "../datas/auc_value_nnet.csv")
